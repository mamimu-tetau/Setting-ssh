...
- [ローカル側（Mac）で鍵を生成する](#ローカル側macで鍵を生成する)
...
パスフレーズはこの鍵を使うときに必要なので忘れないものに。パスワードみたいなもの。<br>
ターミナルでの操作です。<br>
...
$ ls
```
configファイルがあれば`config`と表示され、なければ何も起こりません。<br />
...
[権限追加参考URL](https://rensrv.com/macos/grant-of-auth-httpdconf/)<br /><br />
...
### サーバに公開鍵を登録
***
どなたかのサーバを使わせてもらう場合や「公開鍵くれや」って言われたときはconfigで登録した該当サーバの秘密鍵とペアの **公開鍵（長い英数字の羅列）だけ！** を渡します。<br />
